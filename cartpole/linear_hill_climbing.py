...
for trial in range(1000):
    ...
    best_reward = 0
    best_params = np.random.random([4, 1]) * 2 - 1
    noise_factor = 1
    for episode in range(1000):
        total_reward, params = run_episode(env, parameters=best_params + (np.random.random(
            [4, 1]) * 2 - 1) * noise_scaling * noise_factor)
        ...
        if total_reward > best_reward:
            ...
            noise_factor = max(noise_factor * 1.15, 10)
    eps_to_solve.append(episode)

plt.hist(eps_to_solve, 50, alpha=0.75)
plt.xlabel('Episodes')
plt.ylabel('Frequency')
plt.title('Solving CartPole with Linear Regression with Hill Climbing')
plt.savefig('cartpole_linear_hill climbing.png')
plt.show()